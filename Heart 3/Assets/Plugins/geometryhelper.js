...
public class GeometryHelper {
	...
	static function EndPolys() : GameObject {
	    ...
	    meshContainer.transform.parent = GameObject.Find("Triangles").transform;
	...
	//POLYLINES
	static function CreatePolyline(a : Vector3, b : Vector3, c : Vector3, lineWidth : float, scale : float) : GameObject {
		BeginPolyline();
		AddPolyline(a,b,c);
		EndPolyline(lineWidth, scale);

	}

	static function BeginPolyline(){
		if(mesh != null){
			Debug.LogWarning("Forcing end polys. check that begin/end is matched");
			EndPolyline(0,0);
		}
	...
	static function AddPolyline(a : Vector3, b : Vector3, c : Vector3){
		if(mesh == null){
			Debug.LogWarning("Forcing begin polys. Check that begin/end is matched");
			BeginPolyline();
		}
	...
	static function EndPolyline(lineWidth : float, scale : float) : GameObject {
		mesh.vertices = vertices;
	    mesh.triangles = indices;
		mesh.uv = uvs;
		mesh.colors = colors;
		...
		var lineContainer : GameObject = GameObject("triLine");
	    lineContainer.AddComponent(LineRenderer);
	    var lineRenderer : LineRenderer = lineContainer.GetComponent(LineRenderer);
		lineRenderer.material = new Material (Shader.Find("Diffuse"));
		 lineRenderer.SetColors(Color(1,0,0,1), Color(0,0,1,1));
    	 lineRenderer.SetWidth(lineWidth,lineWidth);
    	 lineRenderer.SetVertexCount(4);
    		lineRenderer.SetPosition(0, vertices[0]*scale);
    		lineRenderer.SetPosition(1, vertices[1]*scale);
    		lineRenderer.SetPosition(2, vertices[2]*scale);
    		lineRenderer.SetPosition(3, vertices[0]*scale);
    		



	   // meshContainer.renderer.sharedMaterial = GetMaterialWithColor(Color.gray);	    
	    lineContainer.transform.parent = GetGeometryParent();
